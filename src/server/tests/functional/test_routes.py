...
def test_selected_info_route(test_client):
    """
    Test the '/node/selected-info' route with a valid ID.
    """
    node_uri = "http://data.15926.org/dm/Thing"
    response = test_client.get(f"/node/selected-info/{node_uri}")
    json_data = response.get_json()
    ...
    assert json_data["id"] == node_uri
    assert "label" in json_data
    assert "has_children" in json_data
    assert "has_parents" in json_data
    assert "dep" in json_data
...
def test_selected_info_route_missing_id(test_client):
    """
    Test the '/node/selected-info' route without providing an ID in the query parameters.
    """
    response = test_client.get("/node/selected-info/")
    json_data = response.get_json()
...
def test_selected_info_route_invalid_id(test_client):
    """
    Test the '/node/selected-info' route with an invalid node ID.
    """
    invalid_node_uri = "http://data.15926.org/dm/NonExistentNode"
    response = test_client.get(f"/node/selected-info/{invalid_node_uri}")
    json_data = response.get_json()
