...
function initialiseGraph(data: any) {
  svg = d3
    ...
    .append('g')
    // move the graph to the starting position
...
function createGraph(data: any) {
  ...
  tree = d3.tree().nodeSize([nodeDistanceX, nodeDistanceY])
  // const tree = d3.tree().size([height, width])

  // Create the d3 hierarchy from the data
...
function updateGraph(source: any) {
  ...
  // link.exit().transition().duration(duration).attr('d', (d: any) => {
...
function renderNodes(source: any) {
  const nodes = source.descendants()
  console.log('Nodes:', nodes)
  nodes.forEach((d: any) => {
    d.y = d.depth * nodeDistanceY
  })
...
function toggleCollapse(node: any) {
  if (!node._children && !node.children) {
    fetchChildren(node).then((returned: any) => {
      // data.value = returned
      node._children = d3.hierarchy(returned).children
