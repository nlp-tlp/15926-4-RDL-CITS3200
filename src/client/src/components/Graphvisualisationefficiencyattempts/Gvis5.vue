...
const hierarchyEndpoint = '/graph/local-hierarchy/'
...
async function fetchChildren(node: any) {
  ...
  try {
    ...
    if (responseData && Array.isArray(responseData.children)) {
      ...
      // })
      // update the dep property of the node
      node.deprecation = responseData.dep ? responseData.dep : null
...
async function getHierarchy(node: any) {
  ...
  try {
    ...
    if (responseData.centre_id === node.id) {
      ...
      // node.centre = true
      // node.hierarchy = responseData.hierarchy
